      ...
      <div className="w-1/3 border-r">
        ...
        <ScrollArea className="h-[calc(100%-4rem)]">
          {conversations.map(convo => (
            <div key={convo.id} onClick={() => setSelectedConversation(convo)} className={`p-4 cursor-pointer hover:bg-zinc-50 ${selectedConversation.id === convo.id ? 'bg-zinc-100' : ''}`}>
              <div className="flex items-center gap-4">
                ...
                <p className="text-xs text-zinc-400">{convo.timestamp}</p>
              </div>
            </div>
          ))}
        </ScrollArea>
              ...
              <div className="space-y-4">
                {messagesData[selectedConversation.id].map(msg => (
                  <div key={msg.id} className={`flex ${msg.sender === 'me' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`p-3 rounded-lg max-w-xs ${msg.sender === 'me' ? 'bg-orange-500 text-white' : 'bg-zinc-200'}`}>
                      {msg.text}
                    </div>
                  </div>
                ))}
