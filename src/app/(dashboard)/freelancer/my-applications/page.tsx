...
import { FileText, Clock, CheckCircle, XCircle, ExternalLink, Loader2 } from 'lucide-react'
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <Tabs defaultValue="all" className="w-full">
        ...
        <TabsContent value="pending" className="mt-4">
          <div className="space-y-3 sm:space-y-4">
            {applications.filter(app => app.status === 'pending' || app.status === 'in_review').length > 0 ? applications.filter(app => app.status === 'pending' || app.status === 'in_review').map((app) => {
              const config = statusConfig[app.status] || statusConfig.pending
              const Icon = config.icon
              return (
                <Card key={app.id} className="overflow-hidden">
        ...
        <TabsContent value="accepted" className="mt-4">
          <div className="space-y-3 sm:space-y-4">
            {applications.filter(app => app.status === 'accepted').length > 0 ? applications.filter(app => app.status === 'accepted').map((app) => {
        ...
        <TabsContent value="rejected" className="mt-4">
          <div className="space-y-3 sm:space-y-4">
            {applications.filter(app => app.status === 'rejected').length > 0 ? applications.filter(app => app.status === 'rejected').map((app) => {
